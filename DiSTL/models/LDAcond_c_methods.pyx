...
def LDA_pass(int[:,:] count, int[:] z, int[:,:] nd, int[:] ndsum,
    ...
    Notes
    -----
    here we allow alpha to vary across documents, this introduces some
    interesting conditioning information

    In particular we are focused on the sentiment measure by

    Predicting Returns with Text Data. Ke, Kelly and Xiu (2019)
    """
