...
def _load_permno_base(permno_f):
...
def _load_permno_ret(permno_f):

    permno_df = pd.read_csv(permno_f)
    permno_df = permno_df.rename(columns={"TICKER": "ticker"})
    permno_df["date"] = pd.to_datetime(permno_df["date"].astype(str))
    permno_df["year"] = permno_df["date"].dt.year
    permno_df["month"] = permno_df["date"].dt.month
    permno_df = permno_df[~permno_df.duplicated(["ticker", "date"])]
...
def load_permno(permno_f, permno_type="base"):
    ...
    if permno_type == "base":
        return _load_permno_base(permno_f)
    elif permno_type == "ret":
        return _load_permno_ret(permno_f)
    else:
        raise ValueError("unsupported permno_type: %s" % permno_type)



def _load_article_doc_id(source_data_dir, date):
...
def _load_month_doc_id(source_data_dir, date):
...
def _load_day_doc_id(source_data_dir, date):
...
def _load_ticker_day_doc_id(source_data_dir, date, permno_df):

    doc_id = pd.read_csv(os.path.join(source_data_dir,
                                      "doc_id_%s.csv" % date),
                         names=["ticker", "date", "doc_id"])
    doc_id["date"] = pd.to_datetime(doc_id["date"])
    doc_id = doc_id.merge(permno_df, on=["ticker", "date"])
#    doc_id = doc_id[["permno", "date", "doc_id"]]

    return doc_id


def _load_ticker_month_doc_id(source_data_dir, date, permno_df):
...
def load_doc_id(source_data_dir, date, doc_id_type="ticker_day",
    ...
    if doc_id_type == "article":
        doc_id = _load_article_doc_id(source_data_dir, date)
    elif doc_id_type == "day":
        doc_id = _load_day_doc_id(source_data_dir, date)
    elif doc_id_type == "month":
        doc_id = _load_month_doc_id(source_data_dir, date)
    elif doc_id_type == "ticker_day":
        doc_id = _load_ticker_day_doc_id(source_data_dir, date, permno_df)
    elif doc_id_type == "ticker_month":
        doc_id = _load_ticker_month_doc_id(source_data_dir, date, permno_df)
    else:
...
def reindex_wrapper(source_data_dir, tmp_dir, out_data_dir, processes,
    ...
    doc_id_type : str
        type of doc id
    permno_type : str
        type of permno file (for if we want to merge ret/vol/etc)
    ...
    # prep dt dates
    dt_dates = [datetime.strptime(d, "%Y_%m") for d in dates]

    # load permno ticker map
    if permno_f:
        permno_df = load_permno(permno_f, permno_type=permno_type)
    else:
        permno_df = None
    ...
    if os.path.exists(count_f):
        os.remove(count_f)
    pool.starmap(counter, [(source_data_dir, tmp_dir, d.strftime("%Y_%m"),
                            doc_id_type,
                            permno_df[((permno_df["year"] == d.year) &
                                       (permno_df["month"] == d.month))])
                           for d in dt_dates])

    # generate reindexed docs and counts
    pool.starmap(reindexer, [(source_data_dir, tmp_dir, out_data_dir,
                              d.strftime("%Y_%m"), ngrams,
                              term_id_offset_dict, doc_id_type,
                              permno_df[((permno_df["year"] == d.year) &
                                         (permno_df["month"] == d.month))])
                             for d in dt_dates])
