"""
generates a ticker-day DTM from the DJ databases where the ticker
index is seperate from the daily index (hence tensor: ticker, day, term)

Note that this is currently considered inferior to DJ_daily_ticker_DTM_query
because the permno-ticker map is dependent on the date (so seperating the
ticker from date is problematic)
"""
...
def gen_company_sql(doc_sql, date):
    ...
    sql = """SELECT tag_label,
                    ...
                    AS new_doc_id
             FROM (SELECT tag_label,
                          new_tag_id,
                          tag_link_djn_company_%s.tag_id,
                          doc_id
                   FROM tmp_ticker
                   INNER JOIN tag_link_djn_company_%s
                   ON tmp_ticker.tag_id = tag_link_djn_company_%s.tag_id
                  ) AS tmp_tag_doc
...
def create_temp_ticker_table(db, schema, company_id_l, data_dir,
                             log_file=None):
    """generates a temporary table containing the tags and tag_ids
    for the corresponding company_id_l
    ...
    sql = """CREATE TABLE tmp_ticker AS
                SELECT tag_label, tag_id,
                       rank() OVER(ORDER BY tag_id) AS new_tag_id
                FROM tag_djn_company
                WHERE tag_label IN (%s)
          """ % (",".join(["'%s'" % t for t in company_id_l]))

    cursor.execute(sql)
    conn.commit()

    copy_sql = "(SELECT tag_label, new_tag_id FROM tmp_ticker)"

    # now dump table to csv
    with open(os.path.join(data_dir, "ticker_id.csv"), "w") as fd:
        cursor.copy_to(fd, copy_sql, sep=",")
...
def drop_temp_ticker_table(db, schema):
    """drops the temp ticker table
...
def gen_count_sql(doc_sql, t_label, ngram, date):
    ...
    sql = """SELECT tmp_doc.new_doc_id,
                    tmp_doc.new_tag_id,
                    tmp_term_%s.new_term_id,
                    sum(count)
                FROM count_%s_%s_%s
                INNER JOIN (%s) AS tmp_doc
...
def count_query(db, schema, date, tag_drop_dict, headline_l, author_l,
                doc_threshold, txt_labels, ngram_l, data_dir,
                log_file = None):
    ...
    doc_sql = gen_company_sql(doc_sql, date)
    doc_sql = gen_threshold_sql(doc_sql, doc_threshold)
    with open(os.path.join(data_dir, "date_id_%s.csv" % date), "w") as fd:
...
if __name__ == "__main__":
    ...
    # create temp ticker table
    create_temp_ticker_table(db, schema, company_id_l, data_dir, log_file)
    ...
    # drop tmp tables
    drop_temp_ticker_table(db, schema)
