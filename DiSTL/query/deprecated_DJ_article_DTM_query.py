"""
generates an article lvl DTM from the DJ postgres database

this is currently out of date and needs to be rewritten
"""
...
def gen_doc_sql(sql, headline_l, author_l, threshold, date):
    """
    ...
    threshold : int
        term count threshold for each article
    date : str
    ...
    sql = """SELECT doc_id,
                    display_date,
                    accession_number,
                    rank() OVER(ORDER BY doc_id) AS new_doc_id
                FROM (%s) AS foo
                WHERE EXTRACT(WEEK FROM display_date) NOT IN (6, 7)
                AND term_count > %d
                AND headline !~* %s
                AND headline !~* %s
          """ % (sql, threshold,
...
def gen_doc_id_sql(doc_sql):
    """generates sql that can be dumped as doc id"""

    sql = """SELECT new_doc_id, display_date, accession_number
...
def create_temp_term_table(db, schema, ngram, threshold, stop_words,
    ...
    sql = """CREATE TABLE tmp_term_%s AS
                SELECT term_label,
                       term_id,
                       rank() OVER(ORDER BY term_id) AS new_term_id
                FROM term_%s
                WHERE doc_count > %d
    ...
    with open(os.path.join(data_dir, "term_%s.csv" % ngram), "w") as fd:
...
def gen_count_sql(doc_sql, ngram, date):
    ...
    sql = """SELECT tmp_doc.new_doc_id, tmp_term_%s.new_term_id, count
                FROM count_%s_%s
                INNER JOIN (%s) AS tmp_doc
                ON count_%s_%s.doc_id = tmp_doc.doc_id
                INNER JOIN tmp_term_%s
                ON count_%s_%s.term_id = tmp_term_%s.term_id
            """ % (ngram, ngram, date, doc_sql, ngram, date,
                   ngram, ngram, date, ngram)

    return sql


def count_query(db, schema, date, tag_drop_dict, headline_l, author_l,
                doc_threshold, ngram_l, data_dir, log_file = None):
    ...
    doc_sql = gen_doc_sql(doc_sql, headline_l, author_l, doc_threshold, date)
    ...
    for ngram in ngram_l:
        t0 = datetime.now()
        count_sql = gen_count_sql(doc_sql, ngram, date)

        count_f = os.path.join(data_dir, "count_%s_%s.csv" % (ngram, date))
...
if __name__ == "__main__":
    ...
    # load finished data
    finished_rows = [r.split(",") for r in
                     open(log_file, "r").read().split("\n")]
    finished = {}
    finished["doc"] = []
    finished["term"] = []
    for ngram in ngram_l:
    ...
    for r in finished_rows:
        if r[0] == "term":
            finished["term"].append(r[1])
        if r[0] == "doc":
            finished["doc"].append(r[1])
        for ngram in ngram_l:
            if r[0] == "count":
                finished[ngram].append(r[2])

    unfinished_ngrams = [ngram for ngram in ngram_l
                         if ngram not in finished["term"]]
    unfinished_dates = [d for d in dates
                        if not all([d in finished[t]
                                    for t in ["doc"] + ngram_l])]
    ...
    pool.starmap(create_temp_term_table, [(db, schema, ngram,
                                           ...
                                           data_dir, log_file)
                                          for ngram in unfinished_ngrams])

    # count (and doc) query
    pool.starmap(count_query, [(db, schema, d, tag_drop_dict, headline_l,
                                author_l, doc_threshold, ngram_l,
                                data_dir, log_file)
                               for d in unfinished_dates])
