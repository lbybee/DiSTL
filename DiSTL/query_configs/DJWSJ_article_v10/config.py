...
project_dir = "/home/lbybee/Dropbox/BK_LB_Projects/topic_model_experiments"
...
dates = [d for d in dates if int(d.split("_")[0]) > 1983]
...
fname = os.path.join(dir_path, "drop_journal")
journal_l = [r for r in open(fname, "r").read().split("\n") if r != ""]
tag_drop_dict = {"djn_subject": subject_l,
                 "djn_product": product_l,
                 "djn_stat": stat_l,
                 "djn_journal": journal_l}
...
#ngram_stop_words_dict = {"%dgram" % n: ["test"] for n in range(1, ngrams + 1)}
#ngram_regex_stop_words_dict = {"%dgram" % n: ["test"] for n in range(1, ngrams + 1)}

# thresholds
ngram_term_threshold_dict = {"%dgram" % n: 500
