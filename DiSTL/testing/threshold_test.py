...
def word_count_sql(date, threshold):
    ...
    sql = """SELECT doc_id,
                    (body_term_count + headline_term_count) AS term_count
             FROM doc_%s
...
def gen_article_count(current_sql):
    ...
    sql = """SELECT count(*) FROM (%s) AS tmp_doc""" % current_sql
    return sql


def gen_unique_ticker_counts(current_sql, date):
    ...
    sql = """SELECT COUNT(term_count), SUM(term_count)
             ...
             GROUP BY tmp_tag_doc.tag_id
          """ % (date, date, date, current_sql)
    return sql
...
def temporary_writer(db, schema, date, threshold, data_directory,
    ...
    threshold_sql = word_count_sql(date, threshold)
    article_sql = gen_article_count(threshold_sql)
    ...
    ticker_sql = gen_unique_ticker_counts(threshold_sql, date)
